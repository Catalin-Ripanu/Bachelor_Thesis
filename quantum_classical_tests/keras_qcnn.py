...
def one_qubit_unitary(bit, symbols):
    ...
    """
    return cirq.Circuit(
        cirq.X(bit) ** symbols[0], cirq.Y(bit) ** symbols[1], cirq.Z(bit) ** symbols[2]
    )
...
def generate_data(qubits):
    ...
    n_rounds = 1000  # Produces n_rounds * n_qubits datapoints.
    excitations = []
    labels = []
    ...
    return (
        tfq.convert_to_tensor(train_excitations),
        np.array(train_labels),
        tfq.convert_to_tensor(test_excitations),
...
quantum_model = tfq.layers.PQC(
    create_model_circuit(cluster_state_bits), readout_operators
)(cluster_state)

qcnn_model = tf.keras.Model(inputs=[excitation_input], outputs=[quantum_model])

# Show the keras plot of the model
tf.keras.utils.plot_model(qcnn_model, show_shapes=True, show_layer_names=False, dpi=70)
...
print(50 * "*" + "Training Loss" + 50 * "*")
print(history.history["loss"][1:])

print(50 * "*" + "Testing Loss" + 50 * "*")
